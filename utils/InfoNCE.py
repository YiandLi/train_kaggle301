...
class InfoNCE(nn.Module):
    ...
    def __init__(self, temperature=0.1, reduction='mean', negative_mode='unpaired', positive_margin=0):
        super().__init__()
    ...
    if negative_keys is not None:
        ...
        if margin > 0:
            ...
            margin_diag = torch.full(
                [logits.shape[0]], fill_value=margin
            )
            device = "cuda" if torch.cuda.is_available() else "cpu"
            logits = logits - torch.diag(margin_diag).to(device)
